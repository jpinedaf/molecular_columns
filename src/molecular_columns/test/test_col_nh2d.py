...
def test_col_nh2d_Q_op_NH2D() -> None:
    ...
    value = col_nh2d.Q_o_NH2D(Tex=[5., 5., 5.]*u.K)
    np.testing.assert_almost_equal(value, 9.69344271)
    #
    value = col_nh2d.Q_p_NH2D(Tex=[5., 5., 5.]*u.K)
...
def test_col_nh2d_values() -> None:
    result = col_nh2d.o_NH2D_thick(Tex=5*u.K, sigma_v=0.2*u.km/u.s, tau=2.0)
    # np.testing.assert_almost_equal(result.to(u.cm**-2).value, 1.32950178e+14)
    assert pytest.approx(result.to(u.cm**-2).value) == 1.32950178e+14

    result = col_nh2d.p_NH2D_thick(Tex=5*u.K, sigma_v=0.2*u.km/u.s, tau=2.0)
    assert pytest.approx(result.to(u.cm**-2).value) == 1.12581955e+14
