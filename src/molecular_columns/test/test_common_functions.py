...
def test_J_nu() -> None:
    ...
    with pytest.raises(UnitsError):
        common_functions.J_nu(Tex=5 * u.K, freq=100 * u.K)  # type: ignore
    assert (
        pytest.approx(common_functions.J_nu(5 * u.K, 100 * u.GHz).to(u.K).value)  # type: ignore
        == 2.97848924
    )
...
def test_tau_nu() -> None:
    with pytest.raises(ValueError):
        common_functions.tau_nu(
            Tex=2 * u.K, Tbg=2.73 * u.K, freq=100 * u.GHz, Tp=1 * u.K  # type: ignore
        )
    assert np.isnan(
        common_functions.tau_nu(
            Tex=0 * u.K, Tbg=2.73 * u.K, freq=100 * u.GHz, Tp=1 * u.K  # type: ignore
        )
    )
    assert (
        pytest.approx(
            common_functions.tau_nu(
                Tex=10 * u.K, Tbg=2.73 * u.K, freq=100 * u.GHz, Tp=1 * u.K  # type: ignore
            )
        )
        == 0.15927105888724097
    )
