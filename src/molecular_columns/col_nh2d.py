...
@u.quantity_input
def p_NH2D_thick(Tex: u.K = 5*u.K, sigma_v: u.km/u.s = 0.2*u.km/u.s, tau: float = 2.0) -> u.cm**-2:
    """
    ...
    Ncol = (8*np.pi*freq**3/c**3)*Q_p_NH2D(Tex=Tex) \
        / A_ul / Q_p_NH2D_i(J_up, Tex=Tex) \
        / (np.exp(h*freq/k_B/Tex) - 1) * TdV
    return Ncol  # .to(u.cm**-2)


@u.quantity_input
def o_NH2D_thick(Tex: u.K = 5*u.K, sigma_v: u.km/u.s = 0.2*u.km/u.s, tau: float = 2.0) -> u.cm**-2:
    """
    ...
    J_up = 2
    TdV = np.sqrt(2*np.pi) * tau * sigma_v
    Ncol = (8*np.pi*freq**3/c**3)*Q_o_NH2D(Tex=Tex) \
        / A_ul / Q_o_NH2D_i(J_up, Tex=Tex) \
        / (np.exp(h*freq/k_B/Tex) - 1) * TdV
    return Ncol  # .to(u.cm**-2)
